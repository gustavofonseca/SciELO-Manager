...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for j in orm.Journal.objects.all().iterator():
            name = j.publisher_name.strip()
            address = j.editor_address.strip()
            city = j.editor_address_city.strip() or j.publication_city.strip()
            state = j.editor_address_state.strip()
            country = j.editor_address_country.strip()
            zipcode = j.editor_address_zip.strip()
            email = j.editor_email.strip()
            phone1 = j.editor_phone1 or ''
            phone2 = j.editor_phone2 or ''

            pub_house = orm.PublishingHouse.objects.create(name=name, address=address,
                    city=city, state=state, zipcode=zipcode, country=country,
                    email=email, phone1=phone1, phone2=phone2)

            j.chief_publishing_house = pub_house
            j.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm.Journal.objects.update(chief_publishing_house=None)
        orm.PublishingHouse.objects.all().delete()
